	...
	<div class="modal-card">
		...
		<section class="modal-card-body">
			...
			<div class="control">
				...
				<label class="radio is-bold">
					<input type="radio" name="foobar" @click="[paymentToggle = true, makePayment( promo.discount != null ? (totalSumInBasket + deliveryCharge.delivery_charge) -  ( (totalSumInBasket + deliveryCharge.delivery_charge) * promo.discount / 100)  : (totalSumInBasket + deliveryCharge.delivery_charge), 'pending', confirmDetail.phone, deliveryTime, confirmDetail.address, currentCartBasket, '{{Auth::user()->name}}', {{Auth::user()->id}}, deliveryCharge.delivery_charge, totalSumInBasket, ' Paid', activeRestaurantName, '{{Auth::user()->email}}') ]">
					Credit Card 
				</label>
